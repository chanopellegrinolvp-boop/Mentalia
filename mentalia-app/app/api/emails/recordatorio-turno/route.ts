...
<div style="max-width:560px;margin:40px auto;background:white;border-radius:16px;overflow:hidden;box-shadow:0 4px 20px rgba(0,0,0,0.08);">
  ...
  <div style="padding:36px 40px;">
    <h2 style="color:#111827;margin-top:0;">Recordatorio de sesión</h2>
    <p style="color:#374151;line-height:1.7;">Hola <strong>${primerNombre}</strong>, te recordamos que mañana tenés una sesión programada.</p>
    ...
    <div style="text-align:center;margin:28px 0;">
      <a href="${meet_url ?? `${SITE_URL}/dashboard/paciente/videollamada`}" style="background:#40916C;color:white;text-decoration:none;padding:14px 32px;border-radius:10px;font-weight:bold;font-size:15px;display:inline-block;">Ir a mi sesión →</a>
    </div>
    <p style="color:#6B7280;font-size:13px;">Si necesitás cancelar, hacelo con al menos 24 horas de anticipación.</p>
  ...
  await fetch("https://api.resend.com/emails", {
    ...
    body: JSON.stringify({ from: FROM, to: paciente_email, subject: "Recordatorio: Tenés una sesión mañana", html }),
  }).catch(() => {});
