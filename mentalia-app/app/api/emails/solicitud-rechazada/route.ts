...
<div style="max-width:560px;margin:40px auto;background:white;border-radius:16px;overflow:hidden;box-shadow:0 4px 20px rgba(0,0,0,0.08);">
  ...
  <div style="padding:36px 40px;">
    <h2 style="color:#111827;margin-top:0;">Solicitud revisada</h2>
    <p style="color:#374151;line-height:1.7;">Hola <strong>${primerNombre}</strong>, el profesional <strong>${profesional_nombre}</strong> revisó tu solicitud de consulta.</p>
    <p style="color:#374151;line-height:1.7;">Lamentablemente no puede atenderte en este momento.${mensaje_rechazo ? ` Dejó el siguiente mensaje:` : ""}</p>
    ${mensaje_rechazo ? `<div style="background:#f9fafb;border-radius:12px;padding:16px 20px;margin:16px 0;border-left:4px solid #D1D5DB;"><p style="margin:0;color:#374151;font-style:italic;">"${mensaje_rechazo}"</p></div>` : ""}
    <p style="color:#374151;line-height:1.7;">Podés buscar otros profesionales disponibles en Mentalia.</p>
    <div style="text-align:center;margin:28px 0;">
      <a href="${SITE_URL}/dashboard/paciente/buscar" style="background:#40916C;color:white;text-decoration:none;padding:14px 32px;border-radius:10px;font-weight:bold;font-size:15px;display:inline-block;">Buscar otro profesional →</a>
    </div>
  ...
  await fetch("https://api.resend.com/emails", {
    ...
    body: JSON.stringify({ from: FROM, to: paciente_email, subject: "Tu solicitud de consulta fue revisada", html }),
  }).catch(() => {});
