...
export default function PagoExito({ searchParams }: { searchParams: { pro?: string } }) {
  return (
    <div className="min-h-screen flex items-center justify-center px-4" style={{ background: "linear-gradient(160deg, #FDFCFA 0%, #D8F3DC 100%)" }}>
      <div className="w-full max-w-md text-center bg-white rounded-3xl shadow-sm border border-gray-100 p-10">
        ...
        <div className="w-16 h-16 rounded-full flex items-center justify-center mx-auto mb-4 text-3xl" style={{ background: "#D8F3DC" }}>
          ✅
        </div>
        <h1 className="text-2xl font-bold text-gray-900 mb-2">¡Pago exitoso!</h1>
        <p className="text-gris mb-6">Tu sesión fue reservada y el pago acreditado. Te enviamos una confirmación por email.</p>
        <div className="space-y-3">
          ...
          <Link href="/dashboard/paciente" className="block w-full py-3 font-semibold rounded-xl border border-gray-200 text-gray-700">
            Ir a mi dashboard
          </Link>
        </div>
      </div>
