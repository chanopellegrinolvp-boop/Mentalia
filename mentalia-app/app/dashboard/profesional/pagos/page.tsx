...
export default async function PagosProfesionalPage() {
    ...
    <div className="p-8 max-w-3xl">
      ...
      <div className="grid grid-cols-2 gap-4 mb-8">
        <div className="bg-white rounded-2xl border border-gray-100 shadow-sm p-5">
          ...
          <div className="text-2xl font-bold text-gray-900">${total.toLocaleString("es-AR")}</div>
          <div className="text-xs text-gris mt-1">Total cobrado (ARS)</div>
        </div>
        <div className="bg-white rounded-2xl border border-gray-100 shadow-sm p-5">
          <div className="text-2xl mb-1">✅</div>
          <div className="text-2xl font-bold text-gray-900">{payments?.filter(p => p.status === "paid").length ?? 0}</div>
          <div className="text-xs text-gris mt-1">Pagos acreditados</div>
        </div>
      </div>
      ...
      <div className="bg-white rounded-2xl border border-gray-100 shadow-sm overflow-hidden">
        ...
        {!payments || payments.length === 0 ? (
          <div className="text-center py-16">
            ...
            <p className="font-semibold text-gray-800">Sin cobros todavía</p>
            <p className="text-sm text-gris mt-1">Tus pagos aparecerán aquí</p>
          </div>
        ) : (
            ...
            {payments.map((p: any) => (
              <div key={p.id} className="flex items-center gap-4 px-6 py-4">
                ...
                <span className="text-xs font-semibold px-2.5 py-1 rounded-full" style={{ background: p.status === "paid" ? "#D8F3DC" : "#fef3c7", color: p.status === "paid" ? "#2D6A4F" : "#d97706" }}>
                  {p.status === "paid" ? "Acreditado" : "Pendiente"}
                </span>
              </div>
            ))}
          </div>
