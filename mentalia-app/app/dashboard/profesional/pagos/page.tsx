...
export default async function CobrosPage() {
  ...
  const totalMes = pagos?.filter(p => p.status === "paid" && p.paid_at && p.paid_at >= inicioMes)
    .reduce((acc, p) => acc + Number(p.amount), 0) ?? 0;
  const totalHistorico = pagos?.filter(p => p.status === "paid")
    .reduce((acc, p) => acc + Number(p.amount), 0) ?? 0;
    ...
    <div className="min-h-screen bg-[#FDFCFA]">
      <header className="bg-white border-b border-gray-100 px-6 py-4">
        <div className="max-w-4xl mx-auto">
          <h1 className="font-semibold text-gray-900">Cobros</h1>
          <p className="text-xs text-gray-400 mt-0.5">Historial de pagos de tus pacientes</p>
        </div>
      </header>
